...
spec:
  strategy:
    type: {{ .strategy | default .defaultValues.strategy }}
...
spec:
  ...
  tls:
    ...
    - hosts:
        {{- range .hosts }}
        - {{ include "otel-demo.name" $ }}-{{ $.name }}.{{ $.runtimeId }}.{{ $.runtimeEnv }}.{{ $.domain }}
        {{- end }}
      {{- with .secretName }}
      secretName: {{ . }}
      {{- end }}
  ...
  rules:
    {{- range .hosts }}
    - host: {{ include "otel-demo.name" $ }}-{{ $.name }}.{{ $.runtimeId }}.{{ $.runtimeEnv }}.{{ $.domain }}
      http:
        paths:
          {{- range .paths }}
          - path:  {{ .path }}
